...
(function (window) {
  ...
  /**
	 ...
	 * 
	 * @param {Object}
	 *            options - Opções que será usada no parametro do objeto
	 */
  ...
  /**
	 * Trata as opções informadas ao instanciar o objeto
	 */
  ...
  /**
	 ...
	 * @param {string}
	 *            tipo - tipo de conversacao que será tilizada.
	 * @param {String} 
  ...
  /**
	 ...
	 * 
	 * Cria a estrutura do chat, montando o html padrão
	 */
  ...
  BBChatCognitivo.prototype._init = function () {
	...
	'</div>'+
	'<div class="button-minimize" onclick="horus.chat.minimizeChat()" title="Minimizar / Maximizar">'+
	...
	'<div class="hcb-sub_header">'+
		'<button class="hcb-close" onclick="horus.chat.alertCloseChat()">Encerrar</button>'+
...
function timeInChat() {
  ...
  /**
	 ...
	 * 
	 * Declaração dos evento disparados na janela do chat
	 */
  ...
  /**
	 ...
	 * Permite que o usuário movimente a tela
	 */
  ...
  /**
	 ...
	 * window_h = variaver para guadar o tamanho da janela depois quando ela míninimizada
	 */
    ...
    if (min === undefined) {
      ...
      if (container && element && resizer) {
        if (!min) {
          ...
          container.style.maxHeight = (window_h - 130) + 'px';
          container.style.height = (window_h - 130) + 'px';
  ...
  /**
	 * Enviar requisição para o cognitivo
	 * 
	 * @param {object}
    ...
    xmlHttp.onreadystatechange = function () {
      if (xmlHttp.readyState == 4) {
        if (xmlHttp.status == 200) {
          ...
          '<p>1 - Verificar e, caso necessário, instalar a cadeia de certificados do Banco do Brasil. Para isso, clique <a href="http://pki.bb.com.br/Tutorial-FF.html" target="_blank" style="text-decoration: underline; color: blue;">aqui</a> e siga o roteiro apresentado.</p>' +
          '<p>2 - Caso a solução acima não resolva o problema, sugerimos limpar o cache do navegador pressionando as teclas (Ctrl + Shift + Del). Certifique-se de que a opção Cache esteja marcada e confirme.</p>';
  ...
  function addMsgAlerta(self, msg) {
    ...
    '<div class="msw-right">Não foi possível conectar-se ao chat.</div>' +
...
/* 	if (!pergunta) {
    ...
    msg += '   <time style="'+ (pergunta ? 'float: right;' : 'float: left;') + '">' + str_data + ' às ' + str_hora + '</time>'
    + ' </div>'
    + '</span>';
    if (pergunta) {
      msg += avatar;
  ...
  BBChatCognitivo.prototype.envFeedNeg = function (idFb) {
    ...
    if( self.options.tipoAcesso === 'IA' ){
    	msg += 'Deseja encaminhar sua dúvida para o gestor?\t<span class="btn-chatFP" id="' + id_sim + '">Sim</span> <span  id="' + id_nao + '" class="btn-chatFN" >Não</span>';
    }
  ...
  BBChatCognitivo.prototype.envGestor = function (enviar, idFb) {
    ...
    if (enviar) {
      ...
      'Assim que sua pergunta for respondida pela equipe de especialistas, você será avisado por e-mail.<br><br>' +
  ...
  /**
	 ...
	 * @param {boolean} envGestor - se o usuário deseja enviar a pergunta para prioridade do gestor.
	 */
